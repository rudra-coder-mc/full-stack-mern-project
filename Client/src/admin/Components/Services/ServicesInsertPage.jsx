...
function ServicesInsertPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (response.data && response.data.success) {
        ...
        setMessage("Services inserted successfully!");
      } else {
        // Handle validation or other errors
        setError(response.data?.message || "Services insertion failed.");
      }
    } catch (error) {
      setError(error.response?.data?.message || "Services insertion failed.");
      console.error(error);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-6">Services Insert</h1>
      <form onSubmit={handleSubmit} className="flex flex-col space-y-4">
        ...
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? "Inserting..." : "Insert Services"}
        </button>
      </form>
    </div>
  );
