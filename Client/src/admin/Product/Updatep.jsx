...
function Updatep(prop) {
  ...
  const handleSubmit = async (event) => {
    ...
    let URL = `http://localhost:4000/api/v1/admin/product/${prop.updateId}`;
    try {
      ...
      if (response.data && response.data.success) {
        ...
        setMessage("Product Updateed successfully!");
        
      } else {
        // Handle validation or other errors
        setError(response.data?.message || "Product Updateion failed.");
      }
    } catch (error) {
      setError(error.response?.data?.message || "Product Updateion failed.");
      console.error(error);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-6">Product Update</h1>
